...
public class DataFetchApi implements Callable<RatesData> {
    ...
    public RatesData FetchApi() throws IOException {
        ...
        OkHttpClient client = new OkHttpClient().newBuilder().readTimeout(30000, TimeUnit.MILLISECONDS).build();
        if(st.contains("AED")){
            try {
        ...
        Request request = new Request.Builder()
                .url(st)
                .addHeader("apikey", "rxVIETLK9ujGIEBDH3TEgCdxqEMeYDyo").get().build();
